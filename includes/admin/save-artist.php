...
function rhp_save_artist_meta($termID) {
    // Replace 'your_nonce_field_name' and 'your_action_name' with your actual values
    if (!isset($_POST['rhp_meta_nonce']) || !wp_verify_nonce($_POST['rhp_meta_nonce'], 'rhp_save_artist_meta_action')) {
    ...
    if (
        ...
        !isset($_POST['rhp_music_url']) ||
        !isset($_POST['rhp_more_info_url']) ||
        !isset($_POST['rhp_artist_image'])
    ) {
    ...
    update_term_meta(
        $termID,
        'music_url',
        esc_url_raw($_POST['rhp_music_url'])
    );
